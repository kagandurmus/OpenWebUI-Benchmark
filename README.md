...
*   **Accuracy Score on the test curriculum:** How many of the 36 tests the model passed correctly. A higher percentage indicates better reasoning and strict instruction adherence.
...
* **Limited Sample Size:** The sample size (N=36) is intentionally kept small to allow a quick comparison of models.
* **LLM Context Window:** During RAG tests, we reduced the number of sales rows by 90% after overflowing the LLM context window several times (1000 --> 100).
* **Tabular Dataset** We strictly used tabular datasets with numeric values to compare the RAG performance individually,
* **Verbosity Penalty** We restricted the maximum output length for each model to 2500 tokens to penalize overthinking in LLMs.
* **Metrics Used** We primarily tested accuracy, TTFT and decoding speed. Researchers can also expand the benchmark with other metrics like PP(Promp Processing speed).

## RAG Test Results
We observed that calculation tests were passed only after activating the thinking option in all LLMs. Both gemma4 and gpt-oss models passed all 4 of RAG tests, whereas the smallest model (qwen4.2-8b) passed 3/4 of all tests (with thinking mode on). The console outputs can be viewed below.


## Future Models to be Explored
* ornith-ai/Ornith-1.5-9B
* Qwen/Qwen3.8-27B
* NVIDIAnemotron-3.5-lightning:30b
